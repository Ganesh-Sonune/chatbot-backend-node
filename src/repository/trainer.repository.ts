...
export abstract class TrainerRepository {
  ...
  abstract existsByNameAndCourseName(name: string, courseName: string,): Promise<boolean>;

  abstract findByNameContainingIgnoreCase(name: string,): Promise<Trainer[]>;

  abstract findByNameContainingIgnoreCaseAndStatusTrue(name: string,): Promise<Trainer[]>;

  abstract findBySpecializationContainingIgnoreCaseAndStatusTrue(specialization: string,): Promise<Trainer[]>;

  abstract searchBySpecialization(specialization: string,): Promise<Trainer[]>;

  abstract findAllExperiences(): Promise<number[]>;

  abstract search(nameFilter: any, emailFilter: any, specializationFilter: any, minExpFilter: any, page: number, size: number,): Promise<Trainer[]>;
